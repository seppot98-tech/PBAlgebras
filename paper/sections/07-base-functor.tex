...
For every $a\in A$ the function $x\mapsto\norm{\pi_x(a)}$ is upper
semicontinuous on $X_A$ and bounded by $\norm{a}$, and each $\pi_x$ is a
PB-morphism onto its image.  Moreover $\Zc(A_x)$ is a quotient of
$C(X_A,\RR)$ by an ideal containing $I_x$, so the base of each fibre is a
single point provided $A_x\ne0$ and the quotient map is surjective on centres.
\end{proposition}

\begin{proof}
Upper semicontinuity of $x\mapsto\norm{a+J_x}$ for a $C(X)$-module is standard:
$\norm{\pi_x(a)}=\inf\{\norm{a-f\cdot a}:f\in C(X_A,\RR),\ f(x)=0,\ 0\le f\le1\}$
up to the usual approximation, an infimum of continuous functions of $x$.
Boundedness and the PB-morphism property are
Proposition~\ref{prop:category}.  The statement about centres follows from
$\pi_x(f\cdot1)=f(x)1$ for $f\in C(X_A,\RR)$.
\end{proof}

\begin{question}[Faithfulness of the bundle]\label{q:faithful-bundle}
Is $\norm{a}=\sup_{x\in X_A}\norm{\pi_x(a)}$ for every $a\in A$?
\end{question}

For $C^*$-algebras the corresponding statement is a theorem
(Dauns--Hofmann), and the usual proof needs a partition-of-unity estimate
$\norm{\sum_if_i\cdot a_i}\le\max_i\norm{a_i}$ for partitions of unity
$\{f_i\}$, which holds automatically there but is an extra hypothesis for a
general Banach module.  A positive answer would give the slogan of this section
its literal meaning:
