...
makes the base functorial (\S\ref{sec:base}), but it cannot distinguish
$T_2(\RR)$ from $M_2(\RR)$ by any choice of constant
(Proposition~\ref{prop:no-constant-separates}).  Everything downstream depends
on which of the two one is willing to give up.  We develop the theory for
$\gauge_1$, because functoriality of the base is the property that makes $\PB$ a
category worth having.
\end{remark}
