...
Table~\ref{tab:constants} collects the critical constants of these and other
finite-dimensional examples, as computed by the verification package; the
conventions and caveats attaching to those numbers are in \S\ref{sec:status}.
The one entry that is not merely numerical is $\kap_{\gauge_1}(T_2(\RR))$, which
Proposition~\ref{prop:T2-constant} bounds below by $\kap^*$ --- the value the
table reports --- with a proof.
...
Whether $T_2(\RR)$ is an \emph{object} of $\PB^{1/4}_{\gauge_1}$ --- i.e.\
whether its funded defect stays within the same constant that $M_2(\RR)$ needs
--- is a quantitative question, and the answer is no:
Proposition~\ref{prop:T2-constant} below shows
$\kap_{\gauge_1}(T_2(\RR))\ge\kap^*>\tfrac14$.  It matters on both counts for
which $T_2(\RR)$ is used: it is the one elementary finite-dimensional witness
for $\PB\not\subseteq\Cstar$, and it is the cone of
Theorem~\ref{thm:t2-cone}.  Both statements therefore hold only above
$\kap^*$.
